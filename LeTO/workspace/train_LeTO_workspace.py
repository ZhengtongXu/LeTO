...
from LeTO.policy.LeTO_policy import LetoPolicy

OmegaConf.register_new_resolver("eval", eval, replace=True)

class TrainLetoWorksapce(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: LetoPolicy = hydra.utils.instantiate(cfg.policy)
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        self.model.reset()
                        for i in range(T):
                            result = self.model.predict_action_eval(
...
def main(cfg):
    workspace = TrainLetoWorksapce(cfg)
    workspace.run()
